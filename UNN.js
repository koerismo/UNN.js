...
/**
 * @typedef {"inpt"|"conv"|"pool"|"full"} UNNLayerType
 * @typedef {"line"|"sigm"|"tanh"|"relu"|"sfmx"} UNNLayerFunc
 * @typedef {[UNNLayerType, UNNLayerFunc, number, number, number]} UNNLayer
 * @typedef {{typ: UNNLayer, ws: number[][]}[]} UNNNetwork
 */
...
/**
 * @param {UNNLayer[]} topo The topology of the network.
 * @param {number} v The max/min initial value of each weight. (Randomly between -v and v)
 * @returns {UNNNetwork}
 */
...
/**
 * @param {UNNNetwork} Ls
 * @param {ArrayLike<number>} Inpt
 * @param {ArrayLike<number>} Os
 */
UNN.GetOutput = function(Ls, Inpt, Os) {
	...
	if (Os.length==0)
		for(let l=0; l<Ls.length; l++) {
			let pa = 0;
			if (l+1 < Ls.length && Ls[l+1].typ[0] == "conv") pa = Ls[l+1].typ[7];
			let len  = UNN.size(Ls[l].typ,0 );
			let plen = UNN.size(Ls[l].typ,pa);
			Os[l] = [new UNN.Array(len),[],pa==0 ? null : new UNN.Array(plen)];
		}

	var FOs = Os[0][0];
	for (let i=0; i<il; i++) FOs[i]=Inpt[i];

	for (let l=1; l<Ls.length; l++) {
 ...
 *
 * @param {UNNNetwork} W
 * @param {ArrayLike<ArrayLike<number>>} Ins
 * @param {ArrayLike<ArrayLike<number>>} Outs
 * @param {{method: "sgd"|"momentum"|"adagrad"|"adadelta", batch_size: number}} prm
 * @param {boolean?} debug
